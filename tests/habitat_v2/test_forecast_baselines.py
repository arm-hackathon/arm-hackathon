...
def history(*, rows: int = 4) -> ForecastHistory:
    ...
    for source in ("primary_sensor_head", "secondary_sensor_head"):
        for zone in range(8):
            for field in ("temperature_k", "pressure_pa", "co2_ppm", "o2_mole_fraction", "relative_humidity"):
                operational.append({"descriptor_id": f"z{zone}/{field}", "unit": "u", "source_kind": source})
    operational += [
        {"descriptor_id": f"branch_airflow_m3_s/z{zone}", "unit": "u", "source_kind": "operational_feedback_instrument"}
        for zone in range(8)
    ]
    operational += [
        {"descriptor_id": name, "unit": "fraction", "source_kind": "operational_feedback_instrument"}
        for name in ("battery_state_of_charge", "oxygen_store_fraction", "sorbent_remaining_fraction")
    ]
    operational += [
        {"descriptor_id": f"unused/{i}", "unit": "u", "source_kind": "operational_feedback_instrument"}
        for i in range(167 - len(operational))
    ]
    targets = tuple(
        {"descriptor_id": f"z{zone}/{field}", "unit": "u"}
        for zone in range(8)
        for field in ("temperature_k", "pressure_pa", "co2_ppm", "o2_mole_fraction", "relative_humidity", "branch_airflow_m3_s")
    ) + tuple({"descriptor_id": n, "unit": "fraction"} for n in ("battery_state_of_charge", "oxygen_store_fraction", "sorbent_remaining_fraction"))
    ...
    for shape in ((rows, 4), (rows, 4), (rows, 287, 4)):
        ...
        array.setflags(write=False)
        other.append(array)
    return ForecastHistory(tuple(range(1, rows + 1)), tuple(float(60 * (i + 1)) for i in range(rows)), numeric, status, other[0], other[1], other[2], layout)
...
def test_persistence_averages_heads_falls_back_and_abstains() -> None:
    ...
    statuses[-1, 41] = (0, 1, 0, 0, 0)  # secondary z0 pressure unavailable -> primary fallback.
    ...
    assert persistence(replace(item, status_f32=statuses), horizon_steps=2).status == "ABSTAIN"
...
def test_linear_ols_persistence_and_no_data_abstention() -> None:
    ...
    one_two = linear_extrapolation(replace(item, status_f32=status), horizon_steps=2, future_times_s=(300, 360))
    assert one_two.values[0, 0] == pytest.approx((item.numeric_f32[-1, 0] + item.numeric_f32[-1, 40]) / 2)
    status[:, (0, 40)] = (0, 1, 0, 0, 0)
    assert linear_extrapolation(replace(item, status_f32=status), horizon_steps=2, future_times_s=(300, 360)).status == "ABSTAIN"


def sample(sample_id: str, cluster: str, x: float, action: float, y: float) -> RidgeSample:
    item = history()
    target = np.full((2, 51), y, dtype=np.float32)
    return RidgeSample(sample_id, cluster, item, np.full(27, action, dtype=np.float32), target, INPUT, TARGET)
...
def test_ridge_action_features_blinding_and_determinism() -> None:
    ...
    samples = tuple(sample(f"s{i}", f"c{i}", float(i), float(i), float(i)) for i in range(4))
...
def test_ridge_rejects_duplicate_identity_and_bad_cluster_split() -> None:
    items = (sample("same", "c1", 0, 0, 0), sample("same", "c2", 1, 1, 1), sample("s3", "c3", 2, 2, 2))
    with pytest.raises(BaselineError):
        fit_direct_ridge(items, horizon_steps=2)
    with pytest.raises(BaselineError):
        fit_direct_ridge(tuple(sample(f"s{i}", "one", i, i, i) for i in range(3)), horizon_steps=2)
