...
class DirectRidgeModel:
    ...
    def predict(self, history: ForecastHistory, proposed_action_f32: np.ndarray) -> np.ndarray:
        ...
        if (history.layout.input_manifest_sha256, history.layout.target_manifest_sha256) != (
            self.input_manifest_sha256,
            self.target_manifest_sha256,
        ):
            raise BaselineError("inference manifest identity drift")
        feature = flatten_features(history, proposed_action_f32, include_action=self.include_action)
        if feature.shape != self.feature_mean.shape:
            raise BaselineError("inference feature binding drift")
        value64 = ((feature.astype(np.float64) - self.feature_mean) / self.feature_scale) @ self.coef + self.target_mean
        return _as_f32(value64.reshape(self.horizon_steps, TARGET_COUNT), "ridge prediction")
...
def _as_f32(value: np.ndarray, label: str) -> np.ndarray:
    ...
    if not np.isfinite(array64).all() or np.max(np.abs(array64), initial=0.0) > np.finfo(np.float32).max:
...
def _validate_history(history: ForecastHistory, *, horizon_steps: int) -> None:
    ...
    if rows not in WINDOW_CANDIDATES or history.numeric_f32.shape != (rows, 194) or history.status_f32.shape != (rows, 167, 5):
        raise BaselineError("history has unsupported D1 window/dimensions")
    if history.mode_f32.shape != (rows, 4) or history.health_f32.shape != (rows, 4) or history.alarm_lifecycle_f32.shape != (rows, 287, 4):
        raise BaselineError("history ancillary tensors have wrong dimensions")
    if (history.layout.input_manifest_sha256, history.layout.target_manifest_sha256) != (INPUT_MANIFEST_SHA256, TARGET_MANIFEST_SHA256):
        raise BaselineError("history manifest identities drift")
    arrays = (history.numeric_f32, history.status_f32, history.mode_f32, history.health_f32, history.alarm_lifecycle_f32)
    if any(array.dtype != np.float32 or not np.isfinite(array).all() for array in arrays):
    ...
    if not np.all((statuses == 0.0) | (statuses == 1.0)) or not np.all(statuses.sum(axis=2) == 1.0):
        raise BaselineError("history availability status is malformed")
    if len(history.layout.operational_descriptors) != 167 or len(history.layout.target_descriptors) != TARGET_COUNT:
...
def _available(history: ForecastHistory, row: int, column: int) -> float | None:
    return float(history.numeric_f32[row, column]) if history.status_f32[row, column, 0] == 1.0 else None


def _target_operational_columns(history: ForecastHistory) -> tuple[tuple[int, int, int | None], ...]:
    ...
    for raw in history.layout.target_descriptors:
        ...
        if "/" in target_id and target_id.rsplit("/", 1)[1] in {
            "temperature_k", "pressure_pa", "co2_ppm", "o2_mole_fraction", "relative_humidity"
        }:
            ...
            feedback = descriptor_map.get((f"branch_airflow_m3_s/{zone}", "operational_feedback_instrument"))
            if feedback is None:
                raise BaselineError("airflow target lacks operational feedback")
            slots.append((feedback, feedback, None))
        elif target_id in {"battery_state_of_charge", "oxygen_store_fraction", "sorbent_remaining_fraction"}:
            feedback = descriptor_map.get((target_id, "operational_feedback_instrument"))
            if feedback is None:
                raise BaselineError("resource target lacks duplicate-validated operational feedback")
            slots.append((feedback, feedback, None))
        else:
...
def _causal_estimates(history: ForecastHistory) -> np.ndarray:
    ...
    values = np.full((history.numeric_f32.shape[0], TARGET_COUNT), np.nan, dtype=np.float64)
    for row in range(values.shape[0]):
        for target, (first, second, _) in enumerate(slots):
            left, right = _available(history, row, first), _available(history, row, second)
            if first == second:
...
def persistence(history: ForecastHistory, *, horizon_steps: int) -> BaselinePrediction:
    ...
    return BaselinePrediction("PREDICTION", _as_f32(np.repeat(latest[None, :], horizon_steps, axis=0), "persistence"))


def linear_extrapolation(history: ForecastHistory, *, horizon_steps: int, future_times_s: Sequence[float]) -> BaselinePrediction:
    _validate_history(history, horizon_steps=horizon_steps)
    ...
    if not np.isfinite(future).all() or (np.diff(future) <= 0).any() or future[0] <= history.completed_times_s[-1]:
        raise BaselineError("future cadence must be finite, increasing and causal")
    observations, times = _causal_estimates(history), np.asarray(history.completed_times_s, dtype=np.float64)
...
def _action(action: np.ndarray) -> np.ndarray:
    ...
    if array.shape != (ACTION_COUNT,) or array.dtype != np.float32 or not np.isfinite(array).all():
        raise BaselineError("proposed action must be finite float32[27]")
    return array


def flatten_features(history: ForecastHistory, proposed_action_f32: np.ndarray, *, include_action: bool) -> np.ndarray:
    _validate_history(history, horizon_steps=2)
    action = _action(proposed_action_f32)
    tensors = (history.numeric_f32, history.status_f32, history.mode_f32, history.health_f32, history.alarm_lifecycle_f32)
    if any(array.dtype != np.float32 or not np.isfinite(array).all() for array in tensors):
...
def _validate_sample(sample: RidgeSample, horizon_steps: int) -> None:
    if type(sample) is not RidgeSample or not sample.sample_id or not sample.family_cluster_id:
        raise BaselineError("ridge sample identity is invalid")
    _validate_history(sample.history, horizon_steps=horizon_steps)
    if (sample.input_manifest_sha256, sample.target_manifest_sha256) != (INPUT_MANIFEST_SHA256, TARGET_MANIFEST_SHA256):
    ...
    if target.shape != (horizon_steps, TARGET_COUNT) or target.dtype != np.float32 or not np.isfinite(target).all():
        raise BaselineError("ridge target must be finite float32[H,51]")


def _fit(features: np.ndarray, targets: np.ndarray, alpha: float) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def _normalised_error(prediction: np.ndarray, truth: np.ndarray, training_targets: np.ndarray) -> float:
    scale = np.percentile(training_targets, 95, axis=0) - np.percentile(training_targets, 5, axis=0)
    supported = scale > 0.0
    if not supported.any():
        raise BaselineError("validation fold has no supported target scale")
    return float(np.mean(np.abs(prediction[:, supported] - truth[:, supported]) / scale[supported]))


def fit_direct_ridge(samples: Sequence[RidgeSample], *, horizon_steps: int, include_action: bool = True) -> DirectRidgeModel:
    if horizon_steps not in HORIZON_CANDIDATES or isinstance(samples, (str, bytes)) or len(samples) < 3:
        raise BaselineError("ridge requires supported H and at least three samples")
    items = tuple(samples)
    for item in items:
        _validate_sample(item, horizon_steps)
    if len({item.sample_id for item in items}) != len(items) or len({item.family_cluster_id for item in items}) < 2:
        raise BaselineError("ridge sample/cluster identities are insufficient or duplicate")
    windows = {item.history.numeric_f32.shape[0] for item in items}
    if len(windows) != 1:
        raise BaselineError("ridge cannot mix window bindings")
    feature = np.stack([flatten_features(x.history, x.proposed_action_f32, include_action=include_action) for x in items]).astype(np.float64)
    ...
    for alpha in RIDGE_ALPHAS:
        ...
        for cluster in cluster_order:
            ...
            training = ~validation
            if not training.any() or not validation.any() or set(np.asarray([x.family_cluster_id for x in items])[training]) & {cluster}:
                raise BaselineError("whole-cluster split leakage")
            mean, scale, target_mean, coef = _fit(feature[training], targets[training], alpha)
            predicted = ((feature[validation] - mean) / scale) @ coef + target_mean
            fold_errors.append(_normalised_error(predicted, targets[validation], targets[training]))
    ...
    return DirectRidgeModel(selected_alpha, include_action, mean, scale, target_mean, coef, next(iter(windows)), horizon_steps, INPUT_MANIFEST_SHA256, TARGET_MANIFEST_SHA256)


def fit_action_aware_and_blinded(samples: Sequence[RidgeSample], *, horizon_steps: int) -> tuple[DirectRidgeModel, DirectRidgeModel]:
    """Fit paired diagnostics from identical samples; only action fields differ."""
    return (fit_direct_ridge(samples, horizon_steps=horizon_steps, include_action=True), fit_direct_ridge(samples, horizon_steps=horizon_steps, include_action=False))
