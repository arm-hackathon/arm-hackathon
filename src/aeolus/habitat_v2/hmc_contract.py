...
def _validate_receipt_schemas(value: Any) -> Mapping[str, Any]:
    ...
    for name in _RECEIPT_NAMES:
        ...
        _exact_string(
            schema["schema_version"], label=f"receipt_schemas.{name}.schema_version"
        )
        fields = _ordered_strings(
            schema["fields"], label=f"receipt_schemas.{name}.fields"
        )
...
def _validate_control_trace(value: Any) -> Mapping[str, Any]:
    ...
    for field in ("header_fields", "footer_fields"):
        fields = _ordered_strings(trace[field], label=f"control_trace.{field}")
        if not fields[-1].endswith("_sha256"):
            raise HMCContractError(f"control_trace.{field} must end in its digest")
    _ordered_strings(
