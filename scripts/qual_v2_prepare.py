...
def verify_packet(p: Path) -> dict[str, Any]:
    ...
    if set(z.files) != required:
        raise ValueError(f"{p}: fields drift {set(z.files)^required}")
    if str(z["schema_version"].item()) != V2_SCHEMA:
        raise ValueError(f"{p}: schema")
    h,a,t = z["history_numeric_f32"], z["operational_available_bool"], z["targets_f32"]
    if h.shape != (5,16,194) or h.dtype != np.float32 or a.shape != (5,16,167) or a.dtype != np.bool_ or t.shape != (5,8,51) or t.dtype != np.float32:
        raise ValueError(f"{p}: tensor shape/dtype")
    if not (np.isfinite(h).all() and np.isfinite(t).all() and np.isfinite(z["proposed_action_f32"]).all()):
        raise ValueError(f"{p}: nonfinite")
    ids=z["continuation_ids"].tolist()
    hashes=z["operational_available_sha256"].tolist()
    if len(ids)!=5 or len(set(ids))!=5 or any(_availability_row_sha256(i,row) != q for i,row,q in zip(ids,a,hashes,strict=True)):
        raise ValueError(f"{p}: availability hash")
    clusters=z["cluster_ids"].tolist()
    if len(set(clusters)) != 1:
        raise ValueError(f"{p}: mixed cluster")
...
def verify_campaign(root: Path, corpus: Path, *, forbid_validation: bool = True) -> dict[str, Any]:
    ...
    for row in rows:
        by[assignment[row["cluster_id"]]].append(row)
...
def cmd_preflight(a: argparse.Namespace) -> None:
    ...
    if output.exists():
        raise ValueError("new preflight output must not exist")
    proto=load_protocol(root)
    design=load_approved_pilot_design(root)
    ...
    receipt["preflight_sha256"]=sha_bytes(canonical_json_bytes(receipt))
...
def cmd_custody(a: argparse.Namespace) -> None:
    ...
    out.write_bytes(canonical_json_bytes(m))
    print(json.dumps({k:m[k] for k in ("packet_count","example_count","availability_true","availability_total","custody_sha256","splits")},indent=2))

p=argparse.ArgumentParser()
sub=p.add_subparsers(required=True)
x=sub.add_parser("preflight")
x.add_argument("--root",required=True)
x.add_argument("--output",required=True)
x.add_argument("--source-commit",required=True)
x.set_defaults(fn=cmd_preflight)
x=sub.add_parser("custody")
x.add_argument("--root",required=True)
x.add_argument("--corpus",required=True)
x.add_argument("--output",required=True)
x.set_defaults(fn=cmd_custody)
a=p.parse_args()
a.fn(a)
