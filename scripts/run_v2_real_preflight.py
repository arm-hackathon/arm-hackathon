...
root=Path('.').resolve(); design=load_approved_pilot_design(root); c=load_forecast_contracts(root)
it=iter_pilot_continuations(design); control=next(it); action=next(x for x in it if x.variant=='ACTION_PROPOSAL')
...
out=root/'out'/'helios-qual-v2'/'v2-resource-preflight-real.json'; out.parent.mkdir(parents=True,exist_ok=True); write_preflight_receipt(out,receipt)
print(receipt)
